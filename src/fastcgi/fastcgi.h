...
{
public:
  cgi_strand(const string& addr,const string& port):m_io_service(),m_strand(m_io_service),m_acceptor(m_io_service),m_socket(m_io_service),m_data(1024,0)
  {
  ...
  void do_some_stackless(boost::system::error_code ec = boost::system::error_code(), std::size_t n = 0)
  {
    reenter(m_coro)
    {
      for(;;)
      {
        yield m_socket.async_read_some(boost::asio::buffer(m_data),boost::bind(&test_strand::do_some_stackless,this,_1,_2));
        yield boost::asio::async_write(m_socket,boost::asio::buffer(m_data,n),boost::bind(&test_strand::do_some_stackless,this,_1,_2));
      }
    }
  }
  ~cgi_strand()
